...
namespace BIMassist.Commands
{
    ...
    public class BoolischeOperationIntersect : IExternalCommand
    {
            ...
            try
            {
                ...
                foreach (Solid solid1 in GeometryElement_1)
                {
                    foreach (Solid solid2 in GeometryElement_2)
                    {
                        try
                        {
                            Solid sd = BooleanOperationsUtils.ExecuteBooleanOperation(solid1, solid2, BooleanOperationsType.Intersect);
                            if (sd.Volume > 0)
                                GeometryElement_Result.Add(sd);
                            else
                            {
                                sd = BooleanOperationsUtils.ExecuteBooleanOperation(solid2, solid1, BooleanOperationsType.Intersect);
                                if (sd.Volume > 0)
                                    GeometryElement_Result.Add(sd);
                            }
                        }
