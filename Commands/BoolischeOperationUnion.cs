...
namespace BIMassist.Commands
{
    ...
    public class BoolischeOperationUnion : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                var GeometryElement_Result = new List<Solid>();
                ...
                foreach (Solid solid1 in GeometryElement_1)
                {
                    foreach (Solid solid2 in GeometryElement_2)
                    {
                        try
                        {
                            var sd = BooleanOperationsUtils.ExecuteBooleanOperation(solid1, solid2, BooleanOperationsType.Union);
                            if (sd.Volume > 0)
                                GeometryElement_Result.Add(sd);
                        }
                        catch (Exception msg)
                        {
                            TaskDialog.Show("Meldung", msg.Message);
                        }
                    }
                }
